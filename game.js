...
stage.liquidBalls = new Kinetic.Animation(function (frame) {
		...
		for (i = 0; i < game.cells.length; i++) {
			...
			if (cell.balls.length == 1) {
				...
				cell.balls[0].setX(amplitude[0] * Math.sin(t * 2 * Math.PI / period[0]) + centerX);
			} else if (cell.balls.length == 2) {
				t = (frame.time + cell.getX() + cell.getY());
				cell.balls[0].setX(amplitude[1] * Math.sin(randMotion + t * 2 * Math.PI / period[1]) + centerX);
				cell.balls[1].setX(centerX - amplitude[1] * Math.sin(randMotion + t * 2 * Math.PI / period[1]));
			} else if (cell.balls.length == 3) {
				t = (frame.time + cell.getX() + cell.getY());
				// circle: (x-a)2 + (y-b)2 = r2
				//		   y = b + √[r^2 - (x-a)^2]
				x1 = 10 * Math.sin(randMotion + t * 2 * Math.PI / period[2] / 2) + cell.centerX
					x2 = -10 * Math.sin(randMotion + t * 2 * Math.PI / period[2] / 2) + cell.centerX
					a = cell.centerX;
				b = cell.centerY;
				r = 10;
				...
				cell.balls[2].setY(cell.centerY - amplitude[1] * Math.sin(t * 2 * Math.PI / period[2]));
			}
...
if(gup("p1")==null||gup("p2")==null)
{
	alert("No players");
	window.location="login.html";
}
players = [getConfig(1), getConfig(2)]//, getConfig(3)];
players[0].name = gup("p1").capitalize();
players[1].name = gup("p2").capitalize();
//players[2].name = 'Men';
